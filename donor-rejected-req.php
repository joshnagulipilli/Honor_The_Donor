...
<div class="requests-main-div">
        <center><h2 style="margin:20px; margin-bottom:30px">REJECTED Requests</h2></center>
        ...
        <?php 
          ...
          $query = "SELECT * FROM `blood-donation-rejected` WHERE username='$username'";
          ...
          if($check){
            while($row = mysqli_fetch_array($query_run))
            {
                ...
                <div class="req-card">
                    ...
                    <center><p style=" display:flex; align-items:center;justify-content:center;margin-top:40px; height:40px; background-color:red; border-radius:5px">REJECTED</p></center>
                </div>
                <?php
            }
          }
        ...
        <?php 
          require("config.php");
          $username=$_SESSION['UserId'];
          $query = "SELECT * FROM `plasma-donation-rejected` WHERE username='$username'";
          $query_run = mysqli_query($con,$query);
          $check= mysqli_num_rows($query_run);
          if($check){
            while($row = mysqli_fetch_array($query_run))
        ...
        <?php 
          require("config.php");
          $username=$_SESSION['UserId'];
          $query = "SELECT * FROM `platelets-donation-rejected` WHERE username='$username'";
          $query_run = mysqli_query($con,$query);
          $check= mysqli_num_rows($query_run);
          if($check){
            while($row = mysqli_fetch_array($query_run))
