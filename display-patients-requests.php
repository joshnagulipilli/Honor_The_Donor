        ...
        <center><h3 style="margin:10px 0px">Requests</h3></center>
            ...
            <?php
                if(isset($_POST['click-here'])){
                    ...
                    $query="SELECT * FROM `blood-request-form` where username='$username'";
                    ...
                    if($check){
                        while($row = mysqli_fetch_array($query_run))
                        {
                            ...
                            <div class="req-card">
                                ...
                                <h5 style="border:2px solid black; min-height:35px; border-radius:5px;padding-left:10px;padding-top:4px"><?php echo $row['num'];?></h5>
                                ...
                                <form action="blood-req-ar.php" method="post" >
            ...
            <?php
                if(isset($_POST['click-here'])){
                    $username=$_POST['timesss'];
                    $query="SELECT * FROM `plasma-request-form` where username='$username'";
                    $query_run = mysqli_query($con,$query);
                    $check= mysqli_num_rows($query_run);
                    if($check){
                        while($row = mysqli_fetch_array($query_run))
                        {
                            ...
                            <div class="req-card">
                                ...
                                <form action="plasma-req-ar.php" method="post" >
                                <input type="hidden" name='timesss' value="<?php echo $row['id'] ?>">
                                <input type="submit" name="reject-plasma" value="DELETE" class="btn btn-primary" style="margin-top:10px;background-color:red ;color:white;border:1px solid red; margin-right:170px; width:100px;height:40px">
                                 <input  type="submit" name="approve-plasma" value="APPROVE" class="btn btn-primary" style="margin-top:10px;background-color:red ;color:white;border:1px solid red; width:100px;height:40px">
                                </form>
                            </div>
                            <?php
                        }
            ...
            <?php
                if(isset($_POST['click-here'])){
                    $username=$_POST['timesss'];
                    $query="SELECT * FROM `platelets-request-form` where username='$username'";
                    $query_run = mysqli_query($con,$query);
                    $check= mysqli_num_rows($query_run);
                    if($check){
                        while($row = mysqli_fetch_array($query_run))
                        {
                            ...
                            <div class="req-card">
                                ...
                                <form action="platelets-req-ar.php" method="post" >
                                <input type="hidden" name='timesss' value="<?php echo $row['id'] ?>">
                                <input type="submit" name="reject-platelets" value="DELETE" class="btn btn-primary" style="margin-top:10px;background-color:red ;color:white;border:1px solid red; margin-right:170px; width:100px;height:40px">
                                 <input  type="submit" name="approve-platelets" value="APPROVE" class="btn btn-primary" style="margin-top:10px;background-color:red ;color:white;border:1px solid red; width:100px;height:40px">
                                </form>
                            </div>
                            <?php
                        }
